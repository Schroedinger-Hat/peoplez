...
import {InitialServerActionState, ServerActionStatus} from "@/app/actions/types";
...
const formSchema = z.object({
    ...
    stripePriceId: z.string()
        .regex(new RegExp('^price_[0-9A-Za-z]+$'), 'You need to provide a Stripe Product Price ID, format: price_xxxxx'),
});
...
export function AdminMembershipCRUDForm({intent, previousValues, onSuccess}: FormProps) {
    const [membershipTemplateState, membershipTemplateAction] = useFormState(
        (intent === CRUDFormIntent.Create) ? createMembershipTemplate : editMembershipTemplate,
        InitialServerActionState
    );
    const form = useForm<z.infer<typeof formSchema>>({
        defaultValues: (intent === CRUDFormIntent.Create)
            ? {}
            : {
                ...previousValues,
    ...
    useEffect(() => {
        if (membershipTemplateState.status === ServerActionStatus.Success && onSuccess) {
            onSuccess(membershipTemplateState)
        }
    }, [membershipTemplateState, onSuccess])

    return <form action={form.handleSubmit(membershipTemplateAction)}>
        <Form {...form}>
            {
                CRUDFormIntent.Edit && <FormField
